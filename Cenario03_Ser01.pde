class Cenario03_Ser01 {
  ...
  PVector valoresToUpdate; //valores para atualizar os dados de interação, são guardados para não intervenir no meio
  //do dessenvolvimento do cenário. Essas variaveis guaram os valores para serem atualizados no final do draw
  ...
  public void updateValores(float v1, float v2, float v3) {
    valoresToUpdate = new PVector(v1, v2, v3);
  }
  public void desenhaSer() {
    ...
    if (valoresToUpdate != null) { // atualiza só quando tem dados para isso
      isWireFrame = true;
      isHelix = true;
      rotateMouse = true;
      pts = 15;//(int) valoresToUpdate.x;
      segments = 40;//(int)valoresToUpdate.z; //v1;
      latheRadius = 10.0;//abs(v3-20.0);//5; //100.0;
      radius =  valoresToUpdate.x;// * noise ((id*50)) ; //+ ( 20.0 * cos (millis() * .001) ) ;//10.0;//40.0;
      amortecimentoMovimento = valoresToUpdate.y;

      valoresToUpdate = null;
    }
  }
  ...
  public void rotacionDeSer() {
    ...
    if (rotateMouse) {
      ...
      valoresRotacion.y = ( map(amortecimentoMovimento, 0, 1, PI, PI*2) );
      valoresRotacion.z = 0 ;  
      //   }
    } else {
  ...
  public void manifestacaoSer03() {
    ...
    for (int i=0; i<=segments; i++) {
      ...
      p5.strokeWeight( (2.5* i/segments)) ;
      float val =  v*i;//cos( i + millis() * .001) ;
      println("j 1  pts: " + pts);
      for (int j=0; j<=pts; j++) {
        vertices[j].x = latheRadius + sin(radians(angle))* (radius * val) ;
      ...
      p5.beginShape(QUAD_STRIP);
      p5.stroke(255);// (255 - (colorV*i % 255) ) * cos(i+millis()*.01) );
      println("j 2  pts: " + pts + "vertices2.length:  " + vertices2.length  );
      for (int j=0; j<=pts; j++) {
        //os vertices centrais
        if (i>0) { //tiene el valor de la vuelta anterior
          //      p5.stroke(0);
